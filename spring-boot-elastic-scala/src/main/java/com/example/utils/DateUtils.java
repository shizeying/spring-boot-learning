...
public class DateUtils {
	...
	public static Long getDate() {
		...
		return Long.valueOf(year) * 10000 + Long.valueOf(month + 1) * 100 + Long.valueOf(date);
	}
	 ...
	 * 		页面大小
	 */
	public static List<Object> pageBySubList(List list, int currentPage, int pagesize) {
	...
	public static String StripHT(String strHtml) {
		if (StringUtils.isNotEmpty(strHtml)) {
			String txt = strHtml.replaceAll("</?[^>]+>", ""); //剔出<html>的标签
	 ...
	 * @return
	 */
	public static Date covertDateStrToDate(String strDate) {
		ParsePosition pos = new ParsePosition(0);
		Date strtodate = new Date();
		try {
			strtodate = formatter.parse(strDate.trim());
	...
	public static String getHHMMSS(String value) {
		...
		return (hour.length() > 1 ? hour : "0" + hour) + ":" + (minute.length() > 1 ? minute : "0" + minute) + ":" + (second.length() > 1 ? second :
				                                                                                                              "0" + second);
	}
